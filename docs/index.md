...
features:
  ...
  - icon: 🎬
    title: 漫剧工坊 · 小说一键改编漫剧
    details: 已定稿章节一键改编竖屏漫剧：画风方向拍板（国漫/二次元/3D/水墨/赛博/真人写实，AI 按书荐前三）→ 画风/人物/场景锚段逐字注入每格分镜保一致 → 切集（每集钩子+卡点）→ 剧本 → 分镜表 → 三轨绘图提示词 → 成片包（配音稿/剪辑清单/SRT字幕）→ 预告片混剪。拿去即梦/可灵/MJ/剪映直接出片。
  - icon: 🎚️
    title: 标签化倾向系统
...
① 画风方向拍板          ② 资产卡(一致性锚)        ③ 切集              ④ 单集流水线                  ⑤ 预告片
   ...
   (AI 按书荐前三)          场景定调段                                                            成片包:配音稿/剪辑清单/SRT
```

**一致性是漫剧的命门，也是这套流水线的核心设计**：画风锚、人物锚、场景锚全部由资产卡统一管理，**逐字注入每一格分镜提示词**（LLM 偶发漏注入时引擎兜底强制拼入）——同一个人不换脸、上百格画风不漂移，这正是「拍出来像一部剧」和「一百张拼贴」的区别。
