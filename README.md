...
- **🧬 去 AI 味 · 双向锚定 + 自愈闭环**——把"读得出是 AI 写的"当头号敌人来打：正向喂**名家 / 预设文风范本**（余华、鲁迅、汪曾祺、金庸、王小波、海明威…任选，也可喂自己的范文、或从已认可章节自动提取）给笔法一个具体的"人"去学，负向用 ✗AI 腔→✓人话 配对反例把套话逐条划掉；定稿再过一道**量化 AI 味门禁**（8 类套话规则 + 句长节奏 / 段落结构统计打分），超标就**定向去味重写→复测→收敛**，没改好自动回退、绝不落更差的版本。正向锚一路铺到续写、片段润色、整章润色每一个入口。*名家文风一律标注"风格参考·非原作节选"，范本为本项目自撰仿写，不含任何原作节选。*
...
- **去 AI 味（双向锚定 + 自愈闭环）**：正向锚——名家 / 预设文风范本（可选余华 / 鲁迅 / 汪曾祺等，或喂自己的范文、从已认可章节提取）+ ✗AI 腔→✓人话 配对反例；检测——8 类套话规则库 + 4 项句式统计（句长 burstiness / 节拍器句组 / 段落结构 / 长重复）打 AI 味指数；门禁——定稿超标自动定向去味重写、复测收敛、没改好即回退，绝不落更差版本。正向锚铺满续写 / 片段润色 / 整章润色所有入口；名家文风均标注"风格参考·非原作节选"（自撰仿写，不嵌原文）
