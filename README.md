...
- **漫剧工坊（衍生改编）**：把已定稿小说改编成竖屏漫剧拍摄手册——美术风格卡 / 角色卡 / 场景卡（画风锚 + 人物锚逐字注入每格分镜，保一致性）→ 集数规划（每集开场钩子 + 结尾卡点）→ 单集剧本（对白 / 口播双模式）→ 分镜表 → 三轨绘图提示词（中文六层 / 英文 / 负面词）→ 成片包（声线选型 + 配音稿 + 剪辑清单 + SRT 字幕），整集导出 Markdown / CSV / JSON / 成片包 / 字幕，拿去即梦 / 可灵 / MJ / 剪映出片；沿用「只产提示词」哲学，不接生成模型
