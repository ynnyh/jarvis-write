...
The hard problem of AI-assisted novel writing isn't producing text — it's keeping a several-hundred-thousand-word story coherent: characters stay in character, foreshadowing gets paid off, and the outline stays editable. Cangshan is not another "one-click novel generator." Text generation is delegated to the LLM; this project builds the **control layer** around it: a temporal story bible for facts, a foreshadowing scheduler for setups and payoffs, a cascading outline engine for edits, and a tag-based tendency system for style — so a long novel stays controllable, revisable, and traceable from the first chapter to the last.
...
- **Whole-book full-text search (Ctrl+Shift+F)**: search prose / outline / bible entities / facts / foreshadowings in one shot — hits come with context snippets and highlighting; click a chapter hit to jump straight to it, a bible hit to the story bible. The FTS5 trigram inverted index is kept in sync by database triggers (no write path can miss it), so "what did this character say before" no longer means scrolling
- **Free-write & dual-track editing**: alongside the default paragraph-as-interface, two whole-chapter modes — **free-write** (a CodeMirror 6 editor with search & replace / undo history / live word count, auto read-only while a generation job runs); **dual-track** (left pane: the current final draft as a read-only reference; right pane: your new draft, with per-paragraph diff stats updating live; saving goes through the same review chain and auto-archives the old version) — whole-chapter rewrites no longer happen inside a bubble
