...
export default function ClipHandcard({ card, mode = "mood", durationS = 30, onExport, onSave }: {
  card: ClipCard;
  mode?: string;
  durationS?: number;
  ...
  const maxBeats = durationS <= 15 ? 2 : 4;
          ...
          {mode === "play" && view.beat_count ? (
            <>
              <div className="field">
                ...
                <input type="number" min={durationS <= 15 ? 1 : 3} max={maxBeats} value={view.beat_count}
                  onChange={(e) => upd({ beat_count: Math.max(durationS <= 15 ? 1 : 3, Math.min(maxBeats, Number(e.target.value) || 1)) })} />
              </div>
