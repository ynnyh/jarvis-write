// 手卡:选定本子的完整物料(寿星资料卡/分镜/三轨/祝福金句/切段),可编辑保存。
import { useState } from "react";
import { WishCard, WishShot } from "../../birthdayApi";
import { toast } from "../../ui/Toaster";
import { errMsg } from "../../pollJob";
import { CopyBtn } from "../../ui/copy";
import { chunkPromptText } from "./shared";

export default function BirthdayHandcard({ card, honoreeName, memories, onExport, onSave }: {
  card: WishCard;
  honoreeName: string;
  memories: string[];
  onExport: (fmt: "md" | "srt" | "json") => void;
  onSave?: (card: WishCard) => Promise<void>;
}) {
  const [editing, setEditing] = useState(false);
  const [draft, setDraft] = useState<WishCard | null>(null);
  const [busy, setBusy] = useState(false);

  function startEdit() {
    // 深拷贝:编辑期间原卡不动,取消可无损还原
    setDraft(JSON.parse(JSON.stringify(card)) as WishCard);
    setEditing(true);
  }
  ...
  async function save() {
    ...
    try {
      ...
      toast.ok("手卡已保存", "切段与回忆点核对已按新分镜重算");
  ...
  const updShot = (seq: number, patch: Partial<WishShot>) => setDraft({
    ...(draft as WishCard),
    shots: (draft as WishCard).shots.map((s) => (s.seq === seq ? { ...s, ...patch } : s)),
  });
  ...
  return (
    <section className="card">
      <div className="card-head">
        ...
        {!editing && <CopyBtn text={card.punchline} label="复制祝福金句" />}
      ...
      {editing ? (
        ...
        <>
          {card.hook_text && <p className="hint"><b>开场钩子:</b>{card.hook_text}</p>}
          ...
          {/* 寿星资料卡:出片时贴在旁边逐条核对(定制片的施工基准) */}
          <div className="sub-summary">
            <div className="card-head mb-2"><b>寿星资料卡</b><span className="muted">出片时对照,回忆段逐条核对</span></div>
            <div className="hint"><b>称呼:</b>{honoreeName}</div>
            {memories.length > 0 && (
              <div className="hint">
                <b>回忆点:</b>
                <ul className="bday-mems">
            ...
            return (
              <div className="sub-summary">
                <div className="card-head mb-2">
                  <b>角色定妆卡(参考图用)</b>
                  <span className="muted">寿星本人优先用真实照片</span>
                </div>
                {films.map((c, i) => (
                ...
                <p className="hint">操作:寿星本人直接用真实照片(回忆杀段走图生视频,提示词已含「保持参考照片特征」);其他角色复制描述 → 文生图出定妆图 → 上传当参考图,样貌就不会漂。</p>
              </div>
            );
          })()}
          {card.lines?.length > 0 && (
                  ...
                  <div key={c.index} className="sub-summary">
                    ...
                    {segText ? (
                      <>
                        ...
                        <p className="hint">粘到图文生视频工具(如 minimax H3):这一段有寿星本人就传真实照片当参考图(提示词已含「保持参考照片面部特征与体型」,脸才不漂);没有合适照片的段,先按定妆卡文生图再传。</p>
                      </>
                    ) : (
          ...
          <p className="hint">出片:按段生成 → 剪映画布拼接 → 压 SRT → 末格加祝福金句字幕卡「{card.punchline}」;想让寿星照片开口说祝福,把台词拿去对口型工具单独做(不进视频提示词)。</p>
          {/* 整片一段生成完时模型自带音频直接可用;要拼接才需要自己配人声 */}
          <p className="hint">
            {(card.chunks?.length ?? 0) === 1
              ? "音频:整片一段出完,不存在段间错位——直接用模型自带的音频最省事;只有祝福金句要一字不差时才自己配一条人声。"
              : "音频:环境音留给模型出,人声与 BGM 整片后期铺(分段各自带人声与音乐,拼接处必然断);BGM 整片铺一条,段边界不换曲。"}
          </p>
        </>
      )}
    </section>
