...
_RULES: list[tuple[str, float, list[tuple[str, str]]]] = [
    ...
    # 空洞比喻:AI 把"说不清的感觉"膨胀成脑内涟漪 / 远方呼唤 / 无形的手这类抽象空镜。
    # 与"比喻连接词癖"(孤立一个"仿佛"只罚 0.6)互补——这里靠共现高危模式做重罚旁路:
    # 连接词/脑心框架 + 抽象喻体近距离共现才命中,精度高,专治用户举的
    # "大脑里的那道涟漪还在,仿佛有人在很远的地方呼唤他的名字"这类无锚点堆砌。
    ("空洞比喻堆砌", 1.5, [
        # 脑内/心内 + 水波/涌动意象:最典型的"内心涟漪"空镜(不需连接词就已是套话)
        (r"(?:脑海|脑中|脑子|大脑|心里|心中|心底|心头|胸口|胸腔|意识|灵魂|神经)"
         r"[里中]?[^。!?!?;;,,、\n]{0,6}"
         r"(?:涟漪|波澜|暗涌|潮水|潮汐|浪潮|涌动|悸动|震颤|余韵|余波|回响|惊涛|骇浪)",
         "脑内/心内抽象波动(涟漪·潮水式空镜)"),
        # 连接词 + 梦呓式施动者 + 虚化动作:"仿佛有人在远处呼唤他的名字"
        (r"(?:仿佛|宛如|彷佛|好似|恍若|犹如|如同|像是)[^。!?!?;;\n]{0,12}"
         r"(?:有人|有什么|有个声音|有道声音|有一个声音|某种力量|某种存在|一双手|一只手)"
         r"[^。!?!?;;\n]{0,10}"
         r"(?:呼唤|召唤|呼喊|低语|耳语|注视|凝视|牵引|叩击|叩响|撕扯|拉扯|吞噬)",
         "梦呓式远方召唤(仿佛有人呼唤)"),
        # 无形的手/力量/线:凭空拟物的抽象施力
        (r"(?:一[只双股道]|某种|一种)?\s*无形的\s*(?:大?手|力量|丝?线|墙|屏障|枷锁|绳索|纽带|压力)",
         "无形的手/力量(空洞拟物)"),
        # 记忆挑逗:"似乎想起什么却又抓不住"——高频记忆套路,抽象且无锚点,专治用户举的
        # "脑海里似乎想起什么,却又抓不住"。"挑逗词→回忆动作→转折→落空"四段近距共现才命中,
        # 精度高。间隔放行逗号/顿号(套路常跨"…什么,却…"这一逗),但不跨句末标点(。!?;)。
        (r"(?:似乎|仿佛|好像|依稀|隐约|恍惚)[^。!?!?;;\n]{0,12}"
         r"(?:想起|记起|忆起|想到|抓住|捕捉|触碰|浮现)[^。!?!?;;\n]{0,14}"
         r"(?:却|又|但|终究|终归|终是|还是|旋即|转瞬)[^。!?!?;;\n]{0,8}"
         r"(?:抓不住|抓不着|想不起|记不起|够不着|捕捉不到|消失|溜走|模糊|散去|飘散|沉了下去)",
         "记忆挑逗(似乎想起又抓不住)"),
    ]),
    ("比喻连接词癖", 0.6, [
