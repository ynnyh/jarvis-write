...
骨架(标题/表格/台词/三轨提示词/切段)由 `media.markdown` 统一提供,这里只写
「这份手卡有哪些块、每块写什么」以及生日专属的两块。
"""
...
_BGM_FALLBACK = "按基调自选:整片铺一条,段边界不换曲"
...
def export_markdown(row: BirthdayWish) -> str:
    ...
    if clip.get("hook_text"):
        md.bullet(f"开场钩子:{clip['hook_text']}")
    if clip.get("punchline"):
        md.bullet(f"**祝福金句字幕卡:{clip['punchline']}**")
    md.add("")

    # 寿星资料卡:出片时贴在旁边逐条核对(定制片的施工基准,不只是备忘)
    md.h2("寿星资料卡")
    md.bullet(
        f"称呼:**{row.honoree_name or '(未填)'}** | 关系:{relationship_label(row.relationship)}"
    )
    if row.milestone:
        md.bullet(f"里程碑:{row.milestone}")
    md.bullet(f"送出人:{row.sender_desc or '(未说明)'}")
    memories = [str(m) for m in (row.memories or []) if str(m or "").strip()]
    if memories:
        md.bullet("回忆点(逐条核对是否被分镜落实):")
    ...
    if shots:
        ...
        if chunks:
            md.segments(chunks)
        md.extend(PLAYGUIDE_LINES)
        md.add("")
        md.quote(
            f"**BGM 风格建议**:{_BGM_BY_TONE.get(row.tone, _BGM_FALLBACK)}"
            "(整片一条,分段自带音乐在接缝处必错拍)。",
            blank=True,
        )
