...
# 灵感工坊是动画短剧,不是「每格一个梗」的梗图串烧。单独给它一套节奏
# 约束,避免通用情绪短片的结构铁律把 30 秒切得过碎。
CLIPS_PLAY_STRUCTURE_RULES = """\
**动画短剧节奏铁律**:
- 先定一个核心前提/笑点,整条片只围绕它发展,不要同时开多个故事;
- 30 秒以 3 个核心看点为目标,最多 4 个(第 4 个只能是很短的尾梗);15 秒控制在 1-2 个主要看点;「看点」是一次有效的升级/反差/回收,不是每个镜头都必须抖包袱;
- 看点之间必须留出反应、动作或环境承接(通常 2-5 秒),让观众看懂因果;连续两个镜头不要都换新梗;
- 优先保持一个主场景,最多一次明确的空间变化;先交代空间和氛围,再让角色在空间里制造笑点;
- 第一格仍要抓人,但可以用「环境中的反常动作」开场;最后一个看点落地后留半拍,不要再追加梗。"""
...
CLIPS_PLAY_CONTEXT = """\
这是**灵感工坊的动画短剧**:为一个「好玩/猎奇/高辨识度」的创意玩法拍一条竖屏短剧,
画风气质要一眼可辨(玩法 directive 已锁气质),允许荒诞、反差、猎奇,但笑点要服务同一个小故事,
不能做成每格都换一个梗的素材拼盘。人物只是叙事入口,环境、光线、声音感和空间关系同样重要。
...
{{
  ...
  "shots": [
    {{"seq": 1, "scene_name": "场景", "characters": ["出场角色名"],
      "character_desc": "该镜头出场角色的定妆级外观:性别/年龄感/身高体态/发型发色/脸型五官/着装款式颜色材质/气质,多角色用「角色名:…」分段,要能直接拿去文生图出参考图,不只写这一刻的动作",
...
6. 每一格必须有 environment_desc 与 atmosphere,不能只写人物动作;至少一格交代完整空间,
   后续镜头也要写清关键陈设/相对位置/光线变化。beat_index 只能引用上面列出的看点,
   continuity 要说明动作或视线怎样接上上一格。
{grounding_rule}"""
