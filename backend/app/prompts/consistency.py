...
3. 时间线与空间(timeline):与契约的剧情时间/章末地点是否冲突?对照全书剧情时间线,
   ...
   本章无跳跃交代却回到当天傍晚)。
4. 世界观法则与宪法(worldrule):是否违反【本书宪法】已确立的世界观硬规则/代价/限制?
   常驻装置(如系统/金手指/信物)是否被写得与其设定矛盾——本该长期存在却凭空失效、
   或在明显该动用它的关键场景里被无故遗忘?倒计时的天数/剩余时间是否与宪法定义冲突
   (已过天数对不上、时间无限拖延)?
5. 环境氛围连续性(ambient):本章开头与上一章章末环境氛围(声音/天气/光线/时段/气味)是否
...
6. 凭空常驻角色与留白冲突(cast):对照【已登场角色名册】与【本书宪法】的「刻意留白」,
...
severity 判定:
- blocker:与硬约束或上章章末状态直接冲突的硬矛盾(读者必然出戏,如凭空冒出「早已常驻」的角色、紧接同一场景环境却无故翻转),必须修正才能放行
- major:大概率矛盾但尚存解释空间
- minor:轻微不一致或措辞瑕疵
