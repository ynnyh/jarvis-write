...
【桥段台账已有标签(防跨章复读:本章如再出现同类桥段,label 必须逐字抄这里的写法,不要另起名)】
{known_motifs}
...
7. motifs(桥段台账,防跨章复读):抽本章最有辨识度、**再写一次就会显重复**的描写母题——
   反复出现/被着重刻画的物件意象(如某件信物的特写)、标志性动作(如扎手、刺胸一类
   自残式动作)、场景收束套路(如「躺下去等天亮」式结尾)、反复套用的氛围写法。
...
# =============== 桥段台账全书扫描(存量书回填)===============
# 老书没有台账数据,防复用要从下一章才开始生效——本扫描把历史章节的描写母题
# 一次性补齐(多章批喂一次调用,控成本)。与 EXTRACTION_PROMPT 的 motifs 字段
# 同一契约:标签短、可检索、同物同名。
MOTIF_SCAN_PROMPT = """\
你是小说连载的"母题管理员"。下面是同一部小说连续几章的正文。请逐章抽出该章
最有辨识度、**再写一次就会显重复**的描写母题,供系统跨章统计复现次数。

【台账已有标签(与它是同一个东西时,label 必须逐字照抄,不要另起名)】
{known_labels}

{chapters_text}

抽取口径:
- 只抽标志性描写母题:反复出现/被着重刻画的物件意象、标志性动作(含自残式动作)、
  场景收束套路(如「躺下去等天亮」式结尾)、反复套用的氛围写法
- label 用 2-8 字的可检索短标签(如:铁锈玫瑰/扎胸膛/躺下等天亮);
  detail 一句话说明,不复述原句
- 推进剧情的一次性动作、只出现一次的普通描写不算;每章最多 3 条,宁缺毋滥

严格按 JSON 输出(不要 markdown 代码块,不要解释):
{{
  "chapters": [
    {{"chapter_number": 章号数字, "motifs": [{{"label": "短标签", "detail": "一句话说明"}}]}}
  ]
}}

只输出上面给出过的章号;某章没有够格的母题,motifs 给空数组,不要漏掉该章。
"""
